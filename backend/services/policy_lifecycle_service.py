...
from backend.storage import sqlite_repository
...
class PolicyLifecycleService:
    ...
    def buy_policy(
        ...
        return sqlite_repository.create_policy(
            user_id=user_id,
    ...
    def list_user_policies(user_id: str) -> List[Dict[str, Any]]:
        policies = sqlite_repository.list_user_policies(user_id)
    ...
    def ensure_policy_active_for_claim(user_id: str, policy_type: str) -> Dict[str, Any]:
        policy = sqlite_repository.get_latest_policy_for_type(user_id=user_id, policy_type=policy_type)
        if policy is None:
